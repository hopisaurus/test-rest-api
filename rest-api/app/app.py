...
hit_counter = Counter('hit_counter','A url request hit counter.', ['method', 'endpoint'])
...
def lookup():
    ...
    try:
        ...
        if validators.domain(hostname):
            ...
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute("INSERT INTO history (route, method, domain, req_body, res_code, res_body) VALUES (%s, %s, %s, %s, %s, %s)", (request.path, request.method, domain, req_body, jsonify(resp).status_code, json.dumps(resp)))
            conn.commit()
            cur.close()
            conn.close()
